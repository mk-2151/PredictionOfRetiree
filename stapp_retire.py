...
def main():
    ...
    if choice == '要約統計量':
        ...
        if 'df' in st.session_state:
            ...
            # 要約統計量の表示
            st_display_table(df.describe(include=int))
    ...
    if choice == 'グラフ表示':
        ...
        if 'df' in st.session_state:
            ...
            xaxis_list = ["退職","月給(ドル)"]
            selected_xaxis = st.sidebar.selectbox("グラフのx軸", xaxis_list)

            # セッションステートに退避していたデータフレームを復元
            df = copy.deepcopy(st.session_state.df)

            # グラフの表示
            st_display_graph(df,selected_xaxis)
    ...
    if choice == '学習と検証':
        ...
        learn_list = ["決定木","ランダムフォレスト",]
        selected_learn = st.sidebar.selectbox("学習の手法", learn_list)

        depth = st.sidebar.number_input('決定木の深さ',min_value=2, max_value=3)
        ...
        if 'df' in st.session_state:
            ...
            st.caption('')
            st.caption('決定木')
            st_display_dtree(clf,None)

            st.subheader('訓練用データでの予測精度')
            col1, col2, col3 = st.columns(3)
            with col1: st.caption("正解率")
            with col2: st.caption("再現率")
            with col3: st.caption("適合率")
            col1, col2, col3 = st.columns(3)
            with col1: st.subheader(f"正解率：{train_scores}")
            with col2: st.caption(recall_score)
            with col3: st.caption(precision_score)
            
            st.subheader('検証用データでの予測精度(未分割）')
            col1, col2, col3 = st.columns(3)
            with col1: st.caption("正解率")
            with col2: st.caption("再現率")
            with col3: st.caption("適合率")
            col1, col2, col3 = st.columns(3)
            with col1: st.caption(accuracy_score)
            with col2: st.caption(recall_score)
            with col3: st.caption(precision_score)
    ...
    if choice == 'About':
        ...
        st.markdown("Build by [NCC Student]")
        st.text("Version 0.3")
        st.markdown("For More Information check out (https://www.ncc-net.ac.jp/?utm_source=google&utm_medium=cpc&utm_campaign=2021&gclid=CjwKCAiAoL6eBhA3EiwAXDom5kuEQxFhcUuAyQQesDqUZ4yJsui1Iw9b2aeVUXaoQF1CXP-rKQQhShoCJ1sQAvD_BwE)")   
